...
void GCS::update_sensor_status_flags()
{
    ...
    if (logger.logging_present() || gps.drivers_should_log()) {  // primary logging only (usually File)
        control_sensors_present |= MAV_SYS_STATUS_LOGGING;
    }
    if (logger.logging_enabled() || gps.drivers_should_log()) {
        control_sensors_enabled |= MAV_SYS_STATUS_LOGGING;
    }
    if (!logger.logging_failed() && gps.logging_healthy()) {
